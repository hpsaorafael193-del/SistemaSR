...
class ExamCatalog {
  ...
  async loadCategories() {
    try {
      ...
      // Se vier categorias explícitas, usa
      if (Array.isArray(data.categorias)) {
        this.app.categorias = data.categorias;
      }
      // Caso contrário, gera a partir dos exames
      else if (Array.isArray(data.exames)) {
        const categoriasMap = {};

        data.exames.forEach((exame) => {
          if (!exame.categoria) return;

          if (!categoriasMap[exame.categoria]) {
            categoriasMap[exame.categoria] = {
              id: exame.categoria,
              nome: this.formatCategoriaNome(exame.categoria),
              icone: "fa-folder",
            };
          }
        });

        this.app.categorias = Object.values(categoriasMap);
      } else {
        this.app.categorias = [];
      }
    } catch (error) {
  ...
  filterExamesByCategory(categoria) {
    if (categoria === "todos") {
      ...
      const filtered = this.app.exames.filter(
        (exame) => exame.categoria === categoria,
      );
  ...
  selectExame(exame) {
    ...
    document.getElementById("titulo-exame-selecionado").textContent =
      exame.nome;
  ...
  showBackToAllExamesButton() {
    ...
    if (!backButton) {
      ...
      if (categoriasContainer && examesContainer) {
        categoriasContainer.parentNode.insertBefore(
          backButton,
          examesContainer,
        );
      }
      ...
      switch (campo.tipo) {
        ...
        case "select":
          const options = campo.opcoes
            ? campo.opcoes
                .map(
                  (opt) => `<option value="${opt.valor}">${opt.label}</option>`,
                )
                .join("")
            : "";
  ...
  filterExames(searchTerm) {
    ...
    if (!searchTerm) {
      allExames.forEach((card) => {
        if (
          this.app.state.exameAtual &&
          card.dataset.id === this.app.state.exameAtual.id
        ) {
