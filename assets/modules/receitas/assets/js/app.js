...
document.getElementById("btn-gerar-pdf").addEventListener("click", () => {
  ...
  const original = document.getElementById("receita-preview");
  const clone = original.cloneNode(true);
  clone.id = "receita-print";

  // força tamanho padrão A4 no clone (não afeta a prévia)
  ...
  setTimeout(() => {
    html2canvas(clone, {
      ...
      width: 794,
      height: 1123,
      windowWidth: 1400, // evita aplicar layout responsivo da tela pequena no render
    })
      .then((canvas) => {
